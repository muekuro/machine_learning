...
class NaiveBayes:
    def __init__(self):
        self.vocabularies = set() # 単語の集団
        self.word_count = {} # {category : { words : n, ...}}
        self.cat_count = {} # {category : n}

    def word_count_up(self, word, cat):
        self.word_count.setdefault(cat, {})
        self.word_count[cat].setdefault(word, 0)
    ...
    def train(self, doc, cat):
        word = getwords(doc)
        for w in word:
            self.word_count_up(w, cat)
        self.cat_count_up(cat)

    def priorprob(self, cat):
        return float(self.cat_count[cat]) / sum(self.cat_count.values())

    def incategory(self, word, cat):
        #あるカテゴリの中に単語が登場した回数を返す
        if word in self.word_count[cat]:
            return float(self.word_count[cat][word])
        return 0.0
    ...
    def wordprob(self, word, cat):
        ...
        (self.incategory(word, cat) + 1.0) / \
            (sum(self.word_count[cat].values()) + \
